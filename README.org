...
- Before each round, the game variant is chosen
- A game consists of a certain number of rounds
- A round consists of 12 tricks
- A trick is the process of playing a card by each player
- After a trick has been played, the trick is assigned to the winner of the trick
- After each round a round summary is shown, which is right now called game summary
- There is a score, that holds the accumulated summary of each round

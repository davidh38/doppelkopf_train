...
def get_ai_hands(game_id):
    ...
    # Get the hands of AI players
    ai_hands = []
    for i in range(1, 4):  # AI players are at indices 1, 2, 3
        ai_hand = [card_to_dict(card) for card in game['hands'][i]]
        ai_hands.append({
            'player': i,
            'hand': ai_hand
        })

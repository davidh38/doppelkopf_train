...
from src.backend.game.doppelkopf import (
    create_game_state, create_card, set_variant, end_game, get_card_order_value,
...
def test_kontra_party_wins_with_special_achievements():
    ...
    if 'diamond_ace_captured' not in game:
    ...
    print("\n=== Final Game Summary ===")
    print(f"Game Over! {'KONTRA'} team wins!")
    print("\nTeam Composition:")
    for i, team in enumerate(game['teams']):
        player_name = f"Player {i}"
        print(f"- {player_name}: {'RE' if team == TEAM_RE else 'KONTRA'}")
    ...
    for i, team in enumerate(game['teams']):
        player_name = f"Player {i}"
        print(f"- {player_name} ({'RE' if team == TEAM_RE else 'KONTRA'}): {player_game_points[i]} game points")

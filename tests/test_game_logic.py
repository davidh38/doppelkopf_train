...
def test_team_reveal():
    ...
    for i, team in enumerate(game['teams']):
        print(f"Player {i}: {TEAM_RE if team == TEAM_RE else TEAM_KONTRA}")
    
    # Find a player with a Queen of Clubs
    queen_of_clubs = create_card(SUIT_CLUBS, RANK_QUEEN, False)
    queen_of_clubs2 = create_card(SUIT_CLUBS, RANK_QUEEN, True)
    
    re_players = []
    for i, hand in enumerate(game['hands']):
        for card in hand:
            if (card['suit'] == SUIT_CLUBS and card['rank'] == RANK_QUEEN and 
                (not card['is_second'] or card['is_second'])):
                re_players.append(i)
    ...
    for i, team in enumerate(game['teams']):
        if i in re_players:
            assert team == TEAM_RE, f"Player {i} should be on team RE"
        else:
            assert team == TEAM_KONTRA, f"Player {i} should be on team KONTRA"
    ...
    for card in game['hands'][re_player]:
        if card['rank'] == RANK_QUEEN and card['suit'] == SUIT_CLUBS:
            queen_in_hand = card
            break
    ...
    # Import get_legal_actions to check legal actions
    from src.backend.game.doppelkopf import get_legal_actions
    print(f"Legal actions after variant selection: {get_legal_actions(game, re_player)}")
    
    # Play the Queen of Clubs
    print(f"\nPlayer {re_player} plays Queen of Clubs")
    result = play_card(game, re_player, queen_in_hand)
    ...
    print(f"Current player: {game['current_player']}")
    print(f"Trick winner: {game.get('trick_winner')}")
    ...
    if game['current_trick'] and len(game['current_trick']) > 0:
        from src.backend.game.doppelkopf import cards_equal
        assert cards_equal(game['current_trick'][0], queen_in_hand), "Queen of Clubs should be in the current trick"
    elif game['tricks'] and len(game['tricks']) > 0 and len(game['tricks'][-1]) > 0:
        from src.backend.game.doppelkopf import cards_equal
        assert cards_equal(game['tricks'][-1][0], queen_in_hand), "Queen of Clubs should be in the last completed trick"
    else:
    ...
    assert game['teams'][re_player] == TEAM_RE, f"Player {re_player} should be on team RE"
...
def test_score_calculation():
    ...
    print(f"Game over, winner: {TEAM_RE}")
    ...
    for i, team in enumerate(game['teams']):
        expected_points = re_points if team == TEAM_RE else kontra_points
        print(f"Player {i} ({TEAM_RE if team == TEAM_RE else TEAM_KONTRA}): Expected {expected_points} points")
